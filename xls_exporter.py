...
def build_type_tree(sheet_name: str, sheet_cells):
    ...
    for col in range(len(option_row_cells)):
        ...
        if field_member.default is not None and field_member.unique:
            raise StructureError('约束冲突', '%s列约束不能同时拥有unique和def,%s' % id, sheet_name, id_row, col)

        # 检查重复定义域
        # if parent.get_member(field_name) is not None:
        #     raise StructureError('重复定义', '重复定义了成员%s' % id, sheet_name, id_row, col)
        ...
        #             raise StructureError('格式错误', '结构体或数组成员必须连续定义在一起,成员%s必须紧跟在%s右侧' % (id, left_side_path), sheet_name, id_row, col)
